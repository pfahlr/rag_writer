...
class TestConfigurablePrompts:
    ...
    def test_generate_single_prompt(self):
        ...
        assert len(jobs) == 1
        assert jobs[0]["task"] == f"You are a content writer creating educational material for '{self.test_params['book_title']}'. Focus on practical applications for {self.test_params['target_audience']}."
...
class TestConfigurationIntegration:
    ...
    def test_config_validation(self):
        ...
        assert config.job_generation.min_prompts_per_section >= 1
        assert config.job_generation.max_prompts_per_section <= 10
        assert config.job_generation.min_prompts_per_section <= config.job_generation.default_prompts_per_section
...
class TestContentTypeLoading:
    ...
    def test_load_all_content_types_on_demand(self):
        ...
        from src.utils.template_engine import get_job_generation_prompt, get_job_generation_rag_context, get_job_templates

        content_types = ['pure_research', 'technical_manual_writer', 'science_journalism_article_writer', 'folklore_adaptation_and_anthology_editor']
        ...
        for content_type in content_types:
            ...
            if content_type == 'technical_manual_writer':
                ...
                try:
                    ...
                    assert False, f"Content type {content_type} should not have job templates"
    ...
    def test_template_rendering_with_context(self):
        ...
        for job in rendered:
            ...
            assert 'machine learning basics' in job['instruction']  # Note: template uses |lower filter
    ...
    def test_content_type_file_structure(self):
        ...
        content_types_dir = Path(__file__).parent.parent.parent / "src" / "tool" / "prompts" / "content_types"
        expected_files = [
            'pure_research.yaml',
...
class TestEdgeCases:
    ...
    def test_zero_prompts(self):
        ...
        jobs = create_fallback_jobs(num_prompts=0, content_type=self.test_params["content_type"], **{k: v for k, v in self.test_params.items() if k != "content_type"})
        assert len(jobs) == 0

    def test_large_number_prompts(self):
        """Test behavior with large number of prompts."""
        jobs = create_fallback_jobs(num_prompts=20, content_type=self.test_params["content_type"], **{k: v for k, v in self.test_params.items() if k != "content_type"})
        assert len(jobs) == 20
    ...
    def test_empty_parameters(self):
        ...
        jobs = create_fallback_jobs(num_prompts=1, content_type=params["content_type"], **{k: v for k, v in params.items() if k != "content_type"})
        assert len(jobs) == 1
