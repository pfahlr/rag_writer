...
from utils.template_engine import (
    ...
    load_content_types_with_templates,
...
def retrieve_rag_context(section_title: str, book_title: str, target_audience: str, rag_key: str, content_type: str = "technical_manual_writer") -> str:
    ...
    try:
        query_template = get_rag_context_query(content_type, ROOT / "src" / "tool" / "prompts" / "content_types.yaml")
    except ValueError as e:
        console.print(f"[yellow]Warning: Could not load RAG context query template ({e}), using fallback[/yellow]")
        query_template = """
...
"""
    ...
    try:
        ...
        if not stdout_content:
            console.print(f"[yellow]Warning: Empty response from RAG subprocess[/yellow]")
            return ""
            ...
            try:
                ...
                if json_match:
                    try:
                        ...
                        console.print(f"[yellow]Warning: Could not parse RAG response as JSON: {stdout_content[:200]}...[/yellow]")
                        return ""
                else:
                    console.print(f"[yellow]Warning: No JSON found in RAG response: {stdout_content[:200]}...[/yellow]")
                    return ""
...
def create_fallback_jobs(
    ...
    try:
        ...
        job_templates = get_job_templates(content_type, ROOT / "src" / "tool" / "prompts" / "content_types.yaml")
        ...
        console.print(f"[yellow]Warning: Failed to load YAML templates ({e}), using legacy hardcoded templates[/yellow]")
        return _create_legacy_fallback_jobs(
            section_title, book_title, chapter_title, section_title_hierarchy,
            subsection_title, subsection_id, target_audience, topic, num_prompts
        )


def _create_legacy_fallback_jobs(section_title: str, book_title: str, chapter_title: str, section_title_hierarchy: str, subsection_title: str, subsection_id: str, target_audience: str, topic: str = "", num_prompts: int = 4) -> List[Dict]:
...
def generate_llm_job_file(
    ...
    try:
        job_generation_prompt_template = get_job_generation_prompt(content_type, ROOT / "src" / "tool" / "prompts" / "content_types.yaml")
    except ValueError as e:
        console.print(f"[yellow]Warning: Could not load job generation prompt template ({e}), using fallback[/yellow]")
...
"""
    ...
    job_generation_prompt = render_string_template(job_generation_prompt_template, context)

    # Add RAG context if requested
    if use_rag and rag_key:
        rag_context_content = retrieve_rag_context(section_title, book_title, target_audience, rag_key, content_type)
        if rag_context_content:
            try:
                rag_context_template = get_job_generation_rag_context(content_type, ROOT / "src" / "tool" / "prompts" / "content_types.yaml")
                rag_context_rendered = render_string_template(rag_context_template, {"rag_context": rag_context_content})
                job_generation_prompt += rag_context_rendered
            except ValueError as e:
                console.print(f"[yellow]Warning: Could not load RAG context template ({e}), using simple format[/yellow]")
...
"""
    ...
    try:
            ...
            try:
                ...
                if json_match:
                    try:
                        ...
                        raise ValueError(f"Could not parse JSON response: {extracted_json[:200]}...")
                else:
                    raise ValueError(f"No JSON found in response: {extracted_json[:200]}...")
            ...
            if isinstance(generated_content, str):
                ...
                try:
                    ...
                    try:
                        ...
                        json_array_match = re.search(r'\[.*\]', extracted_content, re.DOTALL)
                        if json_array_match:
                            try:
                                jobs = json.loads(json_array_match.group(0))
                            except json.JSONDecodeError:
                                raise ValueError(f"Could not parse generated_content as JSON: {extracted_content[:200]}...")
                        else:
                            raise ValueError(f"No JSON array found in generated_content: {extracted_content[:200]}...")
            elif isinstance(generated_content, list):
                jobs = generated_content
            else:
                raise ValueError(f"Unexpected type for generated_content: {type(generated_content)}")
        ...
        if not isinstance(jobs, list) or len(jobs) != num_prompts:
            console.print(f"[yellow]Warning: LLM returned {len(jobs) if isinstance(jobs, list) else 'invalid'} jobs, expected {num_prompts}, using fallback jobs[/yellow]")
            jobs = create_fallback_jobs(section_title, book_title, chapter_title, section_title_hierarchy, subsection_title, section_id, target_audience, topic, num_prompts, content_type)

    except Exception as e:
        console.print(f"[yellow]Warning: Failed to generate LLM jobs ({e}), using fallback[/yellow]")
        jobs = create_fallback_jobs(section_title, book_title, chapter_title, section_title_hierarchy, subsection_title, section_id, target_audience, topic, num_prompts)
...
def generate_job_file(section: OutlineSection, metadata: BookMetadata, sections: List[OutlineSection], use_llm: bool = True, use_rag: bool = False, rag_key: str = None, num_prompts: int = 4, content_type: str = "technical_manual_writer") -> Path:
